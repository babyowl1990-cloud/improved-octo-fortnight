 ...
 *   Left controller (continued):
 *     - squeeze/grip (buttons[1])  -> toggle the car settings menu
 *   Head lean:
 *     - lateral head offset from a calibrated neutral -> leanAmount
 *   Keyboard fallback also maps 'C' to the camera-view toggle and 'M'
 *   to the settings menu toggle. While the settings menu is open,
 *   main.js reinterprets steer/throttle/brake as menu navigation
 *   instead of driving — see main.js for why that's safe (physics is
 *   paused while the menu is up, so there's no conflict).
 *
...
export class VRInputController {
  ...
  constructor(renderer) {
    ...
    this.state = { steer: 0, throttle: 0, brake: 0, handbrake: false, leanAmount: 0, viewToggled: false, menuToggled: false };

    // Rising-edge tracking — a held button shouldn't repeat-fire every frame.
    this._viewButtonWasDown = false;
    this._menuButtonWasDown = false;
  ...
  update() {
    ...
    const menuButtonDown = keyState.menuButtonDown || (xrState?.menuButtonDown ?? false);
    this.state.menuToggled = menuButtonDown && !this._menuButtonWasDown;
    this._menuButtonWasDown = menuButtonDown;
  ...
  _readXRControllers(session) {
    let steer = 0, throttle = 0, brake = 0, handbrake = false, viewButtonDown = false, menuButtonDown = false;
    ...
    for (const source of session.inputSources) {
      ...
      if (source.handedness === 'left') {
        ...
        if (gp.buttons[BUTTON_SQUEEZE]?.pressed) menuButtonDown = true;
      }
    ...
    return { steer, throttle, brake, handbrake, steerActive, throttleActive, brakeActive, leanAmount, viewButtonDown, menuButtonDown };
  }
  ...
  _computeHeadLean() {
    ...
    // Negated for the same reason as VehiclePhysics's steering fix: the
    // driver's real-world right corresponds to world -X under this
    // project's camera convention (CameraRig faces the car's +Z front),
    // so a head offset toward world -X (headX decreasing) is a lean to
    // the driver's actual right, and should read as positive.
    const offset = this._leanNeutralX - headX;

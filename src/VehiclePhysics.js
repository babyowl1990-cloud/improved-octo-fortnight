...
export class VehiclePhysics {
  ...
  constructor(config = {}) {
    ...
    this.corneringStiffnessFront = config.corneringStiffnessFront ?? 100000;
    this.corneringStiffnessRear = config.corneringStiffnessRear ?? 118000; // stiffer rear = less prone to oversteer
    this.tireFriction = config.tireFriction ?? 1.35; // ~1.0 = dry asphalt all-seasons, ~1.3-1.5 = sticky performance tire, ~0.5 = wet, ~0.25 = ice
    ...
    this.handbrakeForceMax = config.handbrakeForceMax ?? 12000; // N, rear-axle-only lock force — scaled up alongside tireFriction/corneringStiffness so it still overpowers rear grip for an intentional slide, not just normal driving
  ...
  update(dt) {
    ...
    // NEGATED here deliberately: given how `heading` combines with this
    // project's Y-up/+Z-forward convention (see world-space note above),
    // a "geometrically positive" wheel angle in the slip-angle math below
    // corresponds to the car yawing toward the world's -X side. Whether
    // that's the driver's left or right depends on which way the camera
    // faces — and CameraRig.js aims the camera at the car's +Z front, under
    // which -X is the driver's LEFT. So without this negation, pushing the
    // input right (input.steer = +1) steered the car left. This is the one
    // place that mapping gets corrected; the slip-angle/force model below
    // is internally consistent and untouched.
    ...
    // NOTE: this is force-only — the rotating-frame coupling (what a
    // textbook bicycle-model derivation adds as "+yawRate*vy" /
    // "-yawRate*vx") is applied separately in step 9 as an exact
    // rotation instead of folded in here. See that step for why.
    const axForce = (fx - fyFront * Math.sin(this.currentSteerAngle)) / this.mass;
    const ayForce = (fyFront * Math.cos(this.currentSteerAngle) + fyRear) / this.mass;
    ...
    // ---- 9. Integrate ----
    // Yaw first, with a hard safety-net clamp. Tire forces are already
    // friction-circle-clamped so yawAccel is bounded, but this catches
    // any edge case (bad tuning, an extreme momentary input) before it
    // can reach the danger zone for the rotation below — no real car
    // spins anywhere near this fast, so it never affects normal driving.
    this.yawRate = clamp(this.yawRate + (yawAccel + leanYawNudge) * dt, -8, 8);
    this.heading += this.yawRate * dt;

    // Apply engine/tire forces first (plain, safe integration — these
    // terms are bounded, forward Euler is fine for them)...
    const vxAfterForce = this.vx + axForce * dt;
    const vyAfterForce = this.vy + ayForce * dt;

    // ...then apply the rotating-frame correction as an EXACT rotation
    // rather than the linearized "+yawRate*vy / -yawRate*vx" terms a
    // textbook derivation gives you. Both agree for small yawRate*dt,
    // but the linear form is a forward-Euler approximation of a
    // rotation, and forward-Euler integration of a rotation is only
    // conditionally stable — it grows the vector's magnitude by
    // sqrt(1+(yawRate*dt)^2) EVERY step. That growth is negligible in
    // normal driving but compounds into an exponential blowup (this is
    // exactly the "spin out and hit 100,000 mph" bug) once yawRate is
    // pushed high enough, e.g. during a hard spin. An exact rotation
    // preserves the vector's magnitude exactly, for any yawRate, so
    // this failure mode isn't just mitigated — it's structurally gone.
    const dTheta = this.yawRate * dt;
    const cosDTheta = Math.cos(dTheta), sinDTheta = Math.sin(dTheta);
    this.vx = vxAfterForce * cosDTheta + vyAfterForce * sinDTheta;
    this.vy = -vxAfterForce * sinDTheta + vyAfterForce * cosDTheta;
    ...
    this.longitudinalG = lerp(this.longitudinalG, axForce / GRAVITY, this.gSmoothing);
    this.lateralG = lerp(this.lateralG, ayForce / GRAVITY, this.gSmoothing);

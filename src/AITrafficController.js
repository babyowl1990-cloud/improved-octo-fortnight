...
export class AITrafficController {
  ...
  _createCarInstance(scene) {
    ...
    // Always-on headlights/taillights. Every OTHER visible thing in this
    // night scene (building windows, streetlight lamps) is emissive —
    // self-illuminating, independent of the deliberately dim ambient
    // light. The car body itself is not; under HighwaySystem's night
    // lighting a plain diffuse body — especially the darker palette
    // entries — can render close enough to the near-black background
    // to be very hard to pick out. Real cars solve this with lights,
    // not brighter paint, so that's the fix here too: headlights ahead
    // (+Z, matching the car's front) and taillights behind, both
    // constantly lit rather than tied to any scene light.
